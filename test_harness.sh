...
edge_assert_equal "Phase 6c.9: freq 5700 -> channel 140" "$(freq_to_channel 5700)" "140"
edge_assert_equal "Phase 6c.10: freq 5720 -> channel 144" "$(freq_to_channel 5720)" "144"
edge_assert_equal "Phase 6c.11: freq 5710 -> channel 142" "$(freq_to_channel 5710)" "142"

# 6c.12: Dual-layout channel parsing
...
for _ in $(seq 1 20); do
	PH7A_RESULT=$( (
		exec 9>"$PH7A_LOCK"
		if flock -n 9; then echo ACQUIRED; else echo REJECTED; fi
	) 2>/dev/null)
...
set +e
(
	unset SSID PASSWORD INTERFACE
	validate_config_values "$EDGE_TMP_DIR/7d-no-ssid"
) >/dev/null 2>&1
R_NO_SSID=$?
(
	unset SSID PASSWORD INTERFACE
	validate_config_values "$EDGE_TMP_DIR/7d-no-pass"
) >/dev/null 2>&1
R_NO_PASS=$?
(
	unset SSID PASSWORD INTERFACE
	validate_config_values "$EDGE_TMP_DIR/7d-no-iface"
) >/dev/null 2>&1
R_NO_IFACE=$?
set -e
